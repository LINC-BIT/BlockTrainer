...
from dianzixuebao.scaling_law.vit_b_16.img_cls.fid.inception import InceptionV3

parser = ArgumentParser(formatter_class=ArgumentDefaultsHelpFormatter)
parser.add_argument("--batch-size", type=int, default=50, help="Batch size to use")
parser.add_argument("--max-num-samples", type=int, default=64)
parser.add_argument(
    "--num-workers",
    type=int,
    help=(
...
def get_activations(
    ...
    pred_arr = np.empty((args.max_num_samples, dims))

    start_idx = 0

    for batch in tqdm(dataloader, leave=False):

        if start_idx >= args.max_num_samples:
            break
...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=1):
    if path.endswith(".npz"):
        ...
        for dir_p in os.listdir(path):
            dir_p = os.path.join(path, dir_p)
            dir_p = pathlib.Path(dir_p)
            files += sorted(
                [file for ext in IMAGE_EXTENSIONS for file in dir_p.glob("*.{}".format(ext))]
            )
...
def main():
    ...
    print(f"#samples: {args.num_max_samples}, FID: {fid_value}")
...
if __name__ == "__main__":
    ...
    if args.save_stats:
        ...
        print(f"{args.max_num_samples},{fid_value}")